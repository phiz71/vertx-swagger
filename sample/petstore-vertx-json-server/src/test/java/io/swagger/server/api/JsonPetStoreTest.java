...
public class JsonPetStoreTest {
    ...
    public static void beforeClass(TestContext context) {
        ...
        vertx = Vertx.vertx();
        dog = new Pet(1L, new Category(1L, "dog"), "rex", new JsonArray(), new JsonArray(), Pet.PetStatus.AVAILABLE);
        orderDog = new Order(1L, 1L, 3, Instant.parse("2017-04-02T11:08:10Z"), Order.OrderStatus.APPROVED, Boolean.TRUE);
    ...
    public void testGetOrderById(TestContext context) {
        ...
        httpClient.getNow(TEST_PORT, TEST_HOST, "/v2/store/order/1", response -> {
            response.bodyHandler(body -> {
                ...
                try {
                    context.assertEquals(orderDog.toString(), jsonObject.toString());
                } catch (Exception e) {
                    context.fail(e);
                }
                async.complete();
